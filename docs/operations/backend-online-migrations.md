...
Administrator bans use an adjacent expand-and-protect pair. `0016_add_admin_user_bans` adds only the one-row `admin_user_ban_state` projection and `admin_user_ban_history`; it does not rewrite `user`, backfill existing accounts, or alter comments and other user-created content. The currently deployed backend can continue reading and writing its old shape after this expansion. Apply `0017_protect_admin_user_bans` before deploying ban-aware code. The second migration gives event timestamps to PostgreSQL, enforces a same-subject linear event chain and an exact event-to-projection snapshot, rejects history rewrites and state-row deletion, and removes all `PUBLIC` privileges.

Grant the runtime role `SELECT` and `INSERT` on `admin_user_ban_history`, `USAGE` and `SELECT` on `admin_user_ban_history_id_seq`, and `SELECT`, `INSERT`, and `UPDATE` on `admin_user_ban_state`. Never grant it history `UPDATE`, `DELETE`, or `TRUNCATE`, or state `DELETE` or `TRUNCATE`. An absent projection row means never moderated; an `unban` row is retained as a version tombstone; and an expired `ban` row is retained for the same reason. Evaluate active state with database time: `established_action = 'ban' AND (ban_expires_at IS NULL OR ban_expires_at > clock_timestamp())`. Do not clear expired rows during rollout. Only the test database owner truncates both tables together during fixture cleanup; production maintenance must preserve the indefinite history and the current version token.

Comment moderation follows the same adjacent expand-and-protect rollout. `0020_add_admin_comment_moderation` only creates `admin_comment_moderation_history` and `admin_comment_moderation_state` with their constraints and indexes; it does not scan, rewrite, or replace constraints on the populated `comments` table. Its referenced-table locks are bounded to the empty-table foreign-key DDL, so the prior backend can keep inserting and reading comments after expansion. Apply `0021_protect_admin_comment_moderation` before deploying comment-moderation writers. Protection gives event timestamps to PostgreSQL, enforces one alternating linear delete/restore chain per comment, checks the current projection against its establishing event, and makes every existing comment row immutable. The existing author foreign key remains unchanged for mixed-version safety; its cascade cannot remove an original because the immutable comment trigger rejects every row deletion atomically, including a deletion initiated by account cleanup.
