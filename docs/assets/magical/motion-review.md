...
| --- | --- | --- | --- | --- |
| LOW | `apps/web/src/components/global/backgrounds/magical-background.css:22` | Artwork immediately static on load | Atmosphere/pattern arrive first; ribbons, emblems and opposing clock rotations follow; palace, wand and character finish the scene | User-requested intentional staged entrance, with transformation and opacity only |
| LOW | `apps/web/src/components/global/backgrounds/magicalMotion.ts:10` | No response to page movement | Nine layers respond to bounded signed scroll velocity; foreground elements trail, clocks counter-rotate, wand tilts; everything returns home | Distinct depth and momentum without perpetual movement |
...
- Desktop real scroll and reversal: 376 sampled frame intervals, median 8.3ms, p95 16.2ms, two frames over 33ms; clock rotation ranged approximately −20.7° to +20.5°. Exact neutral styles restored after settling. This is an observation on the development Mac, not a physical-mobile performance guarantee.
- Phone viewport real scroll and reversal: 317 intervals, median 8.3ms, p95 9.2ms; clock range approximately −6.3° to +6.1°; character, wand and right edge never received scroll styles. All motion settled.
