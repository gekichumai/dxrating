...
Comment moderation exposes a newest-first `/comments` feed with an opaque `(createdAt, id)` keyset cursor, a default page size of 50, and a maximum of 100. Its author, stable-chart, active/deleted, inclusive-lower-date, and exclusive-upper-date filters are combinable and echoed as canonical nullable values for durable administrator deep links. Forward traversal is stable across ordinary newer inserts, but it is a live feed rather than a database snapshot: an immutable comment inserted later with a backdated creation time can appear on a later page. Active bodies are reduced to control-safe previews of at most 240 characters. Deleted rows return only the literal `[deleted]`; the list shape excludes the retained original, internal reason, moderator, event, request-correlation, authentication, session, provider, and network data.

The subject-scoped `/comments/{commentId}` detail procedure is the only comment read that exposes privileged immutable originals and internal moderation reasons. It returns the selected comment, approved author moderation summary, active catalog publication when available, explicit current/historical/unresolved chart context, deterministic root-first thread segments, and separately bounded comment and author-ban histories. Thread segments default to 100 items, have a maximum of 250, and report `complete` or `partial` together with an opaque continuation cursor; every item carries explicit parent, root, and depth fields. A retired or unresolved chart remains visible through its persisted historical reference instead of disappearing. Catalog failures and semantic cursor failures use sanitized typed errors, and all response objects remain strict redaction boundaries.

Delete requires explicit confirmation, the expected nullable state version, a trimmed nonblank internal reason of at most 1,000 characters, and recent primary authentication. Restore requires explicit confirmation and the current nonnull state version, but accepts no reason and does not require recent primary authentication. Both mutations are transactional hierarchy-checked writes. The private contract exposes no comment edit, content update, revision, hard-delete, or bulk-moderation operation, and never returns request-correlation data in comment responses.

Chart-report moderation exposes an open-first/newest-first `/chart-reports` queue with a deterministic opaque keyset cursor, a default page size of 50, and a maximum of 100. Each cursor preserves a fixed traversal boundary: reports created or closed during traversal wait for a fresh queue refresh instead of moving between pages. State, stable chart, field, category, reporter, inclusive-lower/exclusive-upper submission dates, and captured publication revision filters are combinable and echoed as canonical nullable values for durable administrator URLs. Queue rows include only bounded value and explanation previews, immutable captured chart/publication labels, and the approved reporter moderation summary. Authentication providers, sessions, tokens, network metadata, raw source artifacts, and closure notes are excluded.

`/chart-reports/{reportId}` returns the immutable submitted values, explanation, stored HTTP(S) evidence references, exact captured publication/chart context, approved reporter summary, and closure metadata. Its separately named current context either supplies the value from the active publication or identifies the captured stable chart as retired, so publication drift is never mistaken for submitted evidence. Reads never crawl evidence URLs. `/chart-reports/{reportId}/close` accepts only the expected `open` state and an optional trimmed internal note of at most 1,000 characters. The two-state contract has no reopen, delete, edit, assignment, priority, merge, duplicate-link, notification, GitHub synchronization, or bulk-close operation.
