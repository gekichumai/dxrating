SELECT COALESCE((
  SELECT index.indisvalid
    ...
    AND pg_get_indexdef(index.indexrelid, 1, true) = 'lower(btrim(regexp_replace(NORMALIZE(name, NFKC), ''[[:space:]]+''::text, '' ''::text, ''g''::text)))'
  ...
  WHERE index_namespace.nspname = 'public'
    AND index_relation.relname = 'admin_user_search_name_lower_pattern_id_idx'
