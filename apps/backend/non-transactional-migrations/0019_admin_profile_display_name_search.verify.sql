SELECT COALESCE((
  SELECT index.indisvalid
    ...
    AND pg_get_indexdef(index.indexrelid, 1, true) = 'lower(btrim(regexp_replace(NORMALIZE(display_name, NFKC), ''[[:space:]]+''::text, '' ''::text, ''g''::text)))'
    ...
    ) = ARRAY['text_pattern_ops', 'text_ops']::name[]
  FROM pg_index index
  ...
  WHERE index_namespace.nspname = 'public'
    AND index_relation.relname = 'admin_profile_search_display_name_lower_pattern_id_idx'
