...
const RatingCalculatorStatisticsFactItem: FC<{
  ...
  <div className={clsx('flex flex-col items-start gap-2', className)}>
    <div
      className={clsx('font-mono tabular-nums !leading-none -mt-1.5 tracking-tight', {
        'text-4xl': size === 'lg',
        'text-3xl': size === 'md',
      })}
    >
    ...
    <div className="text-sm font-semibold leading-none text-zinc-600 -mt-1">{label}</div>
  </div>
)
...
const Histogram: FC<{
  ...
  const draw = () => {
    ...
    svg
      ...
      .join('rect')
      .attr('class', 'bar1')
      .attr('x', (d) => x(d.x0 ?? 0) - Math.max(0, x(d.x1 ?? 0) - x(d.x0 ?? 0) - 1) / 2)
      .attr('y', (d) => y(d.y1) - 0.5)
    ...
    svg
      ...
      .join('text')
      .attr('class', 'label')
      .attr('x', (d) => x((d.x1 + d.x0) / 2) - Math.max(0, x(d.x1 ?? 0) - x(d.x0 ?? 0) - 1) / 2)
      .attr('y', (d) => y(d.y2) - 5)
      .text((d) => (d.y2 === 0 ? '' : d.y2))
