...
const fetchDivingFish = async (
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Unsatisfactory status code: ${response.status}: ${await response.text()}`,
      );
    }
    ...
    const mapChart = (item: Chart) => {
      return {
        sheetId: canonicalIdFromParts(
          item.title,
          (item.type.toLowerCase() === "sd"
            ? "std"
            : item.type.toLowerCase()) as TypeEnum,
    ...
    toast.success(
      `Successfully imported ${entries.length} records from diving-fish.`,
      {
        id: toastId,
      },
    );
    ...
    toast.error(
      `An error occurred while importing records from diving-fish. Are you sure the username or QQ binded is correct? ${formatErrorMessage(error)}`,
      {
        id: toastId,
      },
    );
...
export const ImportDivingFishDialogContent: FC<{
  ...
  const [divingFishConfig, setDivingFishConfig] =
    useLocalStorage<DivingFishProfile | null>("diving-fish-profile", null);

  const invalidReason = useMemo(() => {
    if (!divingFishConfig) return "missing" as const;
    if (!divingFishConfig.username && !divingFishConfig.qq)
      ...
      <DialogHeader className="mb-2">
        <DialogTitle className="flex flex-col items-start gap-2">
          ...
          <div className="text-sm text-zinc-5">
            Choose one of the following two options and fill in the
            corresponding field. Your settings will be saved in your browser.
          </div>
        </DialogTitle>
      </DialogHeader>
