...
serve(async (_req) => {
  ...
  try {
    const { songId, sheetType, sheetDifficulty, parentId, content } = await _req.json()

    if (songId == null || sheetType == null || sheetDifficulty == null || content == null) {
    ...
    if (parentId) {
      const res = db.selectFrom('comments').select(['id']).where('id', '=', parentId).executeTakeFirst()
      if (!res) {
